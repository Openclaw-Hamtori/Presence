...
Use the test app as a mobile proof producer against the session-provided completion contract.
...
3. Mobile opens the deeplink / QR and loads the link envelope (`session_id`, `service_id`, optional `service_domain`, optional `binding_id`, `flow`, fallback `code`).
4. For `initial_link`, the demo does **not** pre-create a binding hint; the binding is created after backend completion.
5. The test app produces a Presence transport payload with `link_context.completion` copied from the opened session metadata.
...
This app supports QR scanning and deeplink/manual link entry as reference UX, but it is still a proof/reference client rather than productized mobile UI.
